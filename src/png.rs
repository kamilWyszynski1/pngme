...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        ...
        header_bytes.clone_from_slice(&value[0..8]);
        ...
        while value.len() > inx {
            ...
            length_bytes.clone_from_slice(&value[inx..inx + LENGTH_BYTES]);
            let mut len = u32::from_be_bytes(length_bytes) as usize;
            ...
            chunks.push(Chunk::try_from(
                &value[inx..inx + len + LENGTH_BYTES + CHUNK_TYPE_LEN + CRC_LEN],
            )?);
            inx += len + LENGTH_BYTES + CHUNK_TYPE_LEN + CRC_LEN
        }
